document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderTable(page = 1, data = customers) {
    ...
    pageData.forEach((customer, index) => {
      ...
      row.innerHTML = `
          <td class="select-cell"><input type="checkbox" data-id="${customer.id}"></td>
          <td>${start + index + 1}</td>
          <td>${customer.name}</td>
      ...
      checkbox.addEventListener("change", (e) => {
        if (e.target.checked) {
          selectedCustomers.add(parseInt(e.target.dataset.id));
          // console.log(e.target.dataset.id);
        } else {
          selectedCustomers.delete(parseInt(e.target.dataset.id));
        }
        toggleDeleteButton();
      });
  ...
  function handleSearch() {
    ...
    renderTable(1, filteredCustomers);
  }
